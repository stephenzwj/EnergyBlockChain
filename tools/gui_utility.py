...
def validate(input_value, lower_bound, upper_bound, l_inclusive=True, u_inclusive=True, convert_to_integer=False):
    ...
    try:
        ...
        value = False
    if ((u_inclusive and value <= upper_bound) or (not u_inclusive and value < upper_bound)) and (
            (l_inclusive and value >= lower_bound) or (not l_inclusive and value > lower_bound)):
        return value
    return False
...
def create_validation_hook(gui, text_field, description, lower_bound=0.0, upper_bound=1.0, l_inclusive=True,
                           u_inclusive=True, update_data=True, refresh_data=True, convert_to_integer=False,
                           select_list=None, select_numeric=False, ):
    ...
    if select_list is None:
        test = lambda text: validate(text, lower_bound, upper_bound, l_inclusive, u_inclusive,
    ...
    def validation_hook():
        if test(text_field.text()) is False:
            gui.main_window.show_status_message(
                description + ": Input value '" + text_field.text() + "' out of bounds (" + str(
...
class EditTable(QtWidgets.QTableWidget):
    ...
    def copy_fn(self):
        ...
        if len(self.selectedRanges()) > 1:
            self.main_window.show_status_message("Copy command cannot be used with multiple ranges selected.",
                                                 error=True, beep=True)
            return
        ...
        data = np.array(
            [(str(index.data().toString()) if type(index.data()) == QtCore.QVariant else index.data()) for index in
    ...
    def paste_fn(self):
        ...
        try:
            ...
            data = [[(np.complex(val) if val.find("j") > 0 else float(val)) for val in line.split('\t') if len(val) > 0]
            ...
            if len(self.selectedIndexes()) > 0 and len(data) > 0:
                ...
                selection = QtGui.QItemSelection(self.model().index(active_row, active_column),
                                                 self.model().index(active_row + data_rows - 1,
                                                                    active_column + data_columns - 1))
